...
public class BoardManager : MonoBehaviour
{
	...
	private void CreateBoard(Vector2 tileSize)
	{
		...
		for (int x = 0; x < size.x; x++)
		{
			for (int y = 0; y < size.y; y++)
			{
				...
				TileController newTile = Instantiate(tilePrefab, new Vector2(startPosition.x + ((tileSize.x + offsetTile.x) * x), startPosition.y + ((tileSize.y + offsetTile.y) * y)),
	...
	private IEnumerator DestroyAndFillTiles(List<TileController> destroyedTiles, System.Action onCompleted)
	{
		...
		foreach (TileController tile in destroyedTiles)
		{
			...
			Vector2Int targetIndex = new Vector2Int(tileIndex.x,
